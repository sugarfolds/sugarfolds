# Shenfeng Qi

I work at the intersection of finance, law, data, and writing.

My current projects focus on turning messy real-world information into structured, reviewable systems: investment research notes, personal knowledge bases, and local-first data pipelines.

## Featured Project

### [Personal Ledger Pipeline](https://github.com/qishenfeng-sys/personal-ledger-pipeline)

A local-first, source-first data pipeline for messy personal finance exports.

- Normalizes multi-source payment exports into a unified ledger.
- Separates consumption from monthly repayments, internal transfers, matched refunds, and duplicate bank charges.
- Preserves source evidence and routes ambiguous rows into a manual-review queue.
- Uses synthetic sample data only, with privacy boundaries documented in the repository.

## What I Am Building Toward

- **Finance and investment research**: company research, industry mapping, valuation logic, and evidence-based writing.
- **Data workflows**: reproducible pipelines, schema design, data cleaning, reconciliation, and review systems.
- **Law and governance**: data regulation, corporate governance, compliance, and technology policy.
- **Knowledge infrastructure**: personal wiki systems, source-first note management, and long-horizon synthesis.

## Working Style

- I prefer source-first workflows: keep raw evidence traceable, then build structured layers on top.
- I care about explainable rules, especially when data contains ambiguity.
- I use AI tools as collaborators for implementation and review, while keeping judgment, privacy, and source boundaries explicit.

## Tech And Tools

Python · CSV/Excel workflows · data cleaning · Git/GitHub · Markdown knowledge bases · financial statement reading · research writing

